...
class MikrotikService {
    ...
    async createPPPoEUser(user: IUser, plan: any) {
        try {
            ...
            const pppSecret = await client.menu('/ppp/secret').add({
                name: user.mikrotik.username,
                password: user.mikrotik.password,
                service: 'pppoe',
    ...
    async updatePPPoEUser(user: IUser, updates: any) {
        try {
            ...
            // Find existing user
            const pppUsers = await client.menu('/ppp/secret').where('name', user.mikrotik.username);
    ...
    async createSimpleQueue(user: IUser, plan: any) {
        try {
            ...
            const queue = await client.menu('/queue/simple').add({
                ...
                'max-limit': `${plan.speed.download}M/${plan.speed.upload}M`,
                'burst-limit': '0/0',
                'burst-threshold': '0/0',
                'burst-time': '0/0',
    ...
    async getUserTraffic(user: IUser) {
        try {
            ...
            const activeConnections = await client.menu('/ppp/active').where('name', user.mikrotik.username);
            
            // Get queue stats
            const queues = await client.menu('/queue/simple').where('name', user.mikrotik.username);
            
            // Get accounting data
            const accounting = await client.menu('/ip/accounting').call('snapshot');
            
            let trafficData = {
                isActive: activeConnections.length > 0,
                uptime: activeConnections.length > 0 ? activeConnections[0]['uptime'] : '0s',
                downloadRate: '0bps',
                uploadRate: '0bps',
            ...
            if (queues.length > 0) {
                ...
                trafficData.totalDownload = queue['bytes'] ? queue['bytes'].split('/')[0] : '0 bytes';
                trafficData.totalUpload = queue['bytes'] ? queue['bytes'].split('/')[1] : '0 bytes';
            }
    ...
    async removeUser(user: IUser) {
        try {
            ...
            // Remove hotspot user
            const hotspotUsers = await client.menu('/ip/hotspot/user').where('name', user.mikrotik.username);
