...
class EmailService {
  ...
  constructor() {
    ...
    this.adminEmail = process.env.ADMIN_EMAIL || "c.delrosario@fyberblizz.com";
    this.supportEmail =
      process.env.SUPPORT_EMAIL || "c.delrosario@fyberblizz.com";

    this.initTransporter().catch((error) => {
      console.error("⚠️ Email service initialization failed:", error.message);
      console.log("📧 App will continue running without email functionality");
  ...
  private async initTransporter() {
    ...
    for (const config of smtpConfigs) {
      try {
        console.log(
          `📧 Trying ${config.name} SMTP (${config.host}:${config.port})...`,
        );
        ...
        console.log(
          `✅ Email service connected successfully using ${config.name}!`,
        );
        return;
      } catch (error: any) {
    ...
    console.error("❌ All SMTP configurations failed. Email service disabled.");
    this.emailEnabled = false;
    this.initialized = false;
  }

  private async ensureTransporter() {
    if (!this.initialized && this.emailEnabled) {
  ...
  async sendEmail(to: string, subject: string, html: string): Promise<boolean> {
    ...
    try {
      ...
      if (!this.transporter) {
        console.log(`⚠️ No transporter available for email to ${to}`);
        return false;
      }

      const mailOptions = {
        from:
          process.env.EMAIL_FROM || '"Mister Fyber" <noreply@misterfyber.com>',
        to,
        subject,
        html,
      };
...
<head>
    ...
    <style>
        ...
        .details-box h3 { margin-top: 0; color: #28a745; }
        .detail-row { margin: 10px 0; padding: 5px 0; border-bottom: 1px solid #e0e0e0; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="welcome-text">Hello <strong>${user.firstName || user.username}</strong>!</div>
            ...
            <div class="details-box">
                ...
                <div class="detail-row"><span class="detail-label">Full Name:</span><span class="detail-value">${user.firstName || ""} ${user.lastName || ""}</span></div>
                <div class="detail-row"><span class="detail-label">Phone:</span><span class="detail-value">${user.phoneNumber || "Not provided"}</span></div>
                <div class="detail-row"><span class="detail-label">Status:</span><span class="detail-value"><span class="status-badge ${user.status === "active" ? "status-active" : "status-pending"}">${(user.status || "pending").toUpperCase()}</span></span></div>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <hr><p style="color: #666; font-size: 12px;">Mister Fyber</p>
    </div>
</body>
</html>`;
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <div style="text-align: center; margin: 30px 0;">
            <a href="${registerUrl}" style="background-color: #28a745; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; font-weight: bold;">Create Your Account</a>
        </div>
        <p>Use your Application ID: <strong>${application.applicationId}</strong> when registering.</p>
        ${application.adminNotes ? `<div style="margin-top: 20px; padding: 15px; background-color: #e7f3ff; border-left: 4px solid #007bff;"><p><strong>Admin Notes:</strong></p><p>${application.adminNotes}</p></div>` : ""}
        <hr><p style="color: #666; font-size: 12px;">Mister Fyber</p>
    </div>
</body>
</html>`;
    ...
    const adminHtml = `<h2>✅ Application Approved</h2><p>You have approved application #${application.applicationId} for ${application.firstName} ${application.lastName}</p>`;
    await this.sendToAdmin(
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    const adminHtml = `<h2>❌ Application Rejected</h2><p>Application #${application.applicationId} rejected for ${application.firstName} ${application.lastName}</p><p>Reason: ${reason}</p>`;
    await this.sendToAdmin(
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    const adminHtml = `<h2>🧾 Invoice Generated</h2><p>Invoice: ${billing.invoiceNumber || billing._id}<br>Amount: ₱${amount.toFixed(2)}<br>Due: ${dueDate}</p>`;
    await this.sendToAdmin(
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <hr><p style="color: #666; font-size: 12px;">Thank you for being a valued Mister Fyber customer!</p>
    </div>
</body>
</html>`;
    await this.sendEmail(
    ...
    const adminHtml = `<h2>💰 Payment Received</h2><p>Payment of ₱${amount.toFixed(2)} from ${user.email}</p>`;
    await this.sendToAdmin(
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <p>Please pay before the due date to avoid service interruption.</p>
        <hr><p style="color: #666; font-size: 12px;">This is an automated reminder from Mister Fyber.</p>
    </div>
</body>
</html>`;
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Billing Reminder</title></head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <p>Hello ${user.firstName || user.username},</p>
        <div style="padding: 20px; background-color: #f8f9fa; border-left: 4px solid #007bff;"><p>${message}</p></div>
        <p>If you have any questions, please contact our support team.</p>
        <hr><p style="color: #666; font-size: 12px;">Mister Fyber</p>
    </div>
</body>
    ...
    const adminHtml = `<h2>🔄 Account Status Changed</h2><p>${user.firstName || ""} ${user.lastName || ""} (${user.email}) status changed to: ${user.status || "pending"}</p>`;
    await this.sendToAdmin(
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Plan Updated</title></head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
    ...
    const adminHtml = `<h2>📡 User Plan Changed</h2><p>${user.email} changed from ${oldPlan.name} to ${newPlan.name}</p>`;
    await this.sendToAdmin(
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Weekly Service Update</title></head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <hr><p style="color: #666; font-size: 12px;">Stay updated with your internet usage and billing with Mister Fyber.</p>
    </div>
</body>
</html>`;
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Service Interruption</title></head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
        ...
        <hr><p style="color: #666; font-size: 12px;">This is an automated notification from Mister Fyber.</p>
    </div>
</body>
</html>`;
    await this.sendEmail(user.email, "Service Interruption Notice", html);

    const adminHtml = `<h2>⚠️ Service Interruption Reported</h2><p>User: ${user.email}</p><p>Reason: ${reason}</p><p>Estimated Duration: ${duration}</p>`;
