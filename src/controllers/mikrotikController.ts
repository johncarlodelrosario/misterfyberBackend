...
export const getStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!config) {
            return res.status(404).json({ message: 'No active MikroTik configuration' });
        }
...
export const getUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const client = await mikrotikService.connect(config._id.toString()) as any;
...
// @access  Private/Admin
export const getActiveConnections = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const config = await MikrotikConfig.findOne({ isActive: true });
...
// @access  Private
export const getUserTraffic = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
// @access  Private/Admin
export const applyPlanToUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
// @access  Private/Admin
export const getInterfaces = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const config = await MikrotikConfig.findOne({ isActive: true });
